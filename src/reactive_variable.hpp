...
template <class T>
class reactive_variable<T> : readonly_reactive_variable<T>, subject<T>, observer_node_parent<T>
{
private:
    T current;
...
public:
    ...
    explicit reactive_variable<T>(T& value): complete_state(0), error(std::runtime_error(""))
    {
    ...
    reactive_variable<T>(T& value, bool subscribe_with_init) : subscribe_with_init(subscribe_with_init), complete_state(0),
    ...
    reactive_variable<
        T>(T& value, const bool call_on_value_changing_in_constructor, const bool subscribe_with_init) : subscribe_with_init(subscribe_with_init),
