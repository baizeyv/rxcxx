...
#ifndef ZIP_OPERATOR_HPP
#define ZIP_OPERATOR_HPP
#include <functional>
#include <memory>
#include <mutex>
#include <queue>
...
namespace rxcxx::operators {
    namespace zip_internal {
        ...
        auto prepare_subscribers_impl(TPL tpl, std::function<void()> nextfn, std::shared_ptr<std::mutex> mtx,
                                      stream_controller<RTYPE>, const VALQ &) {
            return tpl;
        }

        template<std::size_t N, typename TPL, typename RTYPE, typename VALQ, typename OB, typename... ARGS>
        auto prepare_subscribers_impl(TPL tpl, std::function<void()> nextfn, std::shared_ptr<std::mutex> mtx,
                                      stream_controller<RTYPE> sctl, const VALQ &valq, OB ob,
                                      ARGS... args) {
        ...
        auto prepare_subscribers(std::function<void()> nextfn, std::shared_ptr<std::mutex> mtx,
                                 stream_controller<RTYPE> sctl, const VALQ &valq, ARGS... args) {
        ...
        auto zip_main(ARGS... args) {
            return [args...](auto src) {
                ...
                return observable<>::make_observable<rtype>([src, args...](observer<rtype> s) {
                    auto sctl = stream_controller<rtype>(s);
    ...
    auto zip(X x, ARGS... args) {
        return [x, args...](auto src) {
            using FRET = lambda_return_type_t<X>;
            return observable<>::make_observable<FRET>([x, args..., src](observer<FRET> s) {
                auto sctl = stream_controller<FRET>(s);
                ...
                obs.subscribe(sctl.template make_observer<Item>(
                    [sctl, x](auto, Item r) {
