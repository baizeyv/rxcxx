...
public:
    take_while_observer(abs_observer<T>* observer, std::function<bool(T&)> predicate) : observer(observer), predicate(predicate) {}
...
template<class T>
class take_while final : public abs_observable<T>, public operator_ {
private:
    /**
     * * source 被观察者指针
     */
